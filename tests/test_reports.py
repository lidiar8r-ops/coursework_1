...
from src.reports import decorator_write_with_args, spending_by_weekday


@pytest.mark.parametrize("date_input, expected_result", [("invalid-date-format", True), ("2023-09-04", False)])
...
def test_out_of_range_transactions():
    out_of_range_data = pd.DataFrame(
        {"Дата платежа": pd.to_datetime(["2023-06-01"]), "Сумма платежа": [-100], "Валюта платежа": ["USD"]}
    )
...
def test_grouping_and_avg_calculation():
    ...
    result = (
        df.groupby("день_недели")["Сумма платежа_RUB"]
        .mean()
        .reset_index()
        .assign(средние_траты=lambda x: abs(x["Сумма платежа_RUB"]))
        .drop(columns=["Сумма платежа_RUB"])
        .sort_values(by="день_недели")
    )

    expected_result = pd.DataFrame({"день_недели": [1, 2, 3], "средние_траты": [100.0, 200.0, 300.0]})
